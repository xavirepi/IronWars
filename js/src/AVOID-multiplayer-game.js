class MultiPlayerGame {
    ...
    start() {
        ...
        if (!this.interval) {
            this.interval = setInterval(() => {
                ...
                this.draw();
                this.move();
                this.player.checkCollisions(); // Player 1
                this.player2.checkCollisions(); // Player 2
                this.timeCount++;

                if (this.timeCount % time_FPS === 0) {
                    this.time--;
                    this.checkTime();
                    this.player.gameWon(); // Player 1
                    this.player2.gameWon(); // Player 2
                }
    ...
    checkCollisions() {
        if (this.bubbles.some(bubble => this.player.collidesWith(bubble))) {
            // this.gameOver();
        } else if (this.bubbles.some(bubble => this.player.collidesWith(bubble))) {
            // this.gameOver();
        }
